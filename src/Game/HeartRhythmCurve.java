...
class HeartRhythmCurve {
        ...
        for (int cycle = 0; cycle < totalCycles; cycle++) {
            for (double i = 0; i < cycleLength; i += step) {
                ...
                if(cycle==1||cycle==3){
                    ...
                    int x1 = (int) (5 * (currentI / cycleTime) + cycle * cycleLength);
                    int yInt = (int) (y + baseline);
                    curvePoints.add(new Point(x1, yInt));
                }else{
                    if (i >= 0 && i < 60) {
    ...
    /*private void generateArrhythmia() {
        ...
        // Ustalanie długości cykli z przerwami
        for (int cycle = 0; cycle < totalCycles; cycle++) {
            double currentCycleLength = cycleLength;  // Standardowa długość cyklu
            if (cycle == 1 || cycle == 3) {
                // Przykładowo, dla 1 i 3 cyklu, wstawiamy przerwy, ale długość cyklu nie zmienia się
                currentCycleLength = cycleLength;  // Długość cyklu pozostaje ta sama
            }

            // Generowanie punktów w obrębie cyklu
            for (double i = 0; i < currentCycleLength; i += step) {
                double shiftedI = i + cycle * cycleLength; // Przesunięcie o długość cyklu

                // Jeśli cykl 1 lub 3 - dodajemy przerwy w określonych przedziałach
                if (cycle == 1 || cycle == 3) {
                    double currentI = i;
                ...
                // Rysowanie punktu
                int x1 = (int) (5 * (shiftedI / cycleTime));  // Przesunięcie X w zależności od cyklu
                int yInt = (int) (y + baseline);
                curvePoints.add(new Point(x1, yInt));
            }
        }
